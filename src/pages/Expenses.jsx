...
const Expenses = () => {
    ...
    const [transactionsList, setTransactionsList] = useState(JSON.parse(localStorage.getItem('gastos') ?? '[]'))
    const ref = useRef()
                ...
                <div className='relative flex flex-col mx-2 mt-12 lg:mt-0'>
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className='flex flex-row gap-2 w-full'>
                            ...
                            <button
                                type="button"
                                className='button'
                                onClick={() => addExpenses({ data: transactionsList, action: setTransactionsList })}>Guardar</button>
                            <button
                                type="button"
                                className='button'
                                onClick={() => setIsDialogOpen((state) => !state)}>Ver registros</button>
                        </div>
                    </form>
                    {(Array.isArray(transactionsList)
                        ...
                        && <Table
                            ...
                            withMaxHeight={true}
                            buttons={[
                                (rows, index) => <button key='delete' className='button' onClick={() => removeRow(index)}>Borrar</button>
                            ]}
                        />}
                </div>
            </div>
