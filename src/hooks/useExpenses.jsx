...
const useExpenses = (executeUseEffect = true) => {
  ...
  useEffect(() => {
    if (user && executeUseEffect) {
      let today = Date.now()
      getExpenses({ start: today, end: today })
    }
  }, [user])
  ...
  const addExpenses = async ({ data, action }) => {
    if (data.length > 0) {
      ...
      try {
        const addingExpenses = toast.loading('Guardando gastos...', {
          id: 'addingExpenses',
        })
        ...
        toast.dismiss()
        toast.error('Error al guardar gastos', { id: 'expensesError' })
      }
    } else {
