...
const NewUser = ({ isOpen, setIsOpen }) => {
  ...
  const [filteredBusinessData, setFilteredBusinessData] =
    useState(initialBusinessData)

  const handleUserChange = (e) => {
    if (e.target.name === "businessState") {
      const businessArray = data.companies.filter(
        (business) => e.target.value === business.state
      )
    ...
    if (e.target.name === "businessCity") {
      const businessArray = data.companies.filter(
        (business) => e.target.value === business.city
      )
      setFilteredBusinessData(businessArray)
    }
    ...
    if (e.target.name === "businessName") {
      const businessArray = data.companies.filter(
        (business) => e.target.value === business.name
      )
      setFilteredBusinessData(businessArray)
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Select
          ...
          data={Object.fromEntries(
            data.roles.map((item) => [item.roleName, item.id])
          )}
        />
        <Select
          ...
          onChange={handleUserChange}
          data={Object.fromEntries(
            data.companies.map((item) => [item.state, item.id])
          )}
        />
        {userData.businessState !== "" && (
          <Select
            ...
            onChange={handleUserChange}
            data={Object.fromEntries(
              filteredBusinessData.map((item) => [item.city, item.id])
            )}
          />
        )}
        {userData.businessCity !== "" && (
          <Select
            ...
            onChange={handleUserChange}
            data={Object.fromEntries(
              filteredBusinessData.map((item) => [item.name, item.id])
            )}
          />
        )}
        {userData.businessName !== "" && (
          <Select
            ...
            onChange={handleUserChange}
            data={Object.fromEntries(
              filteredBusinessData.map((item) => [item.address, item.uuid])
            )}
          />
        )}
